		...
		if ($this->validate()) {
			try {
				...
				if (extension_loaded('imagick')) {
					...
					$srcImg->clear();
					$icon->writeImages(Yii::getAlias('@assetsPath/x/'.$rndFilename.'.ico'), true);
					$icon->clear();
				} else {
					...
					foreach ($this->dimensions as $dimension) {
						$tmpFiles[] = Yii::getAlias('@assetsPath/x/'.$rndFilename.'.'.$dimension.'.png');
						exec('convert -scale '.$dimension.' '.$srcImg.' '.Yii::getAlias('@assetsPath/x/'.$rndFilename).'.'.$dimension.'.png');
					}
					exec('convert '.implode(' ', $tmpFiles).' '.Yii::getAlias('@assetsPath/x/'.$rndFilename.'.ico'));
				...
				if ($this->email) {
					Yii::$app->mailer
						...
						->attach(Yii::getAlias('@assetsPath/x/'.$rndFilename.'.ico'), ['fileName' => 'favicon.ico'])
						->send()
					;
				}
