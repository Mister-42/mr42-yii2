		...
		if ($this->validate()) {
			try {
				...
				if (extension_loaded('imagick')) {
					...
					$srcImg->clear();
					$icon->writeImages(Yii::getAlias('@webroot/assets/x/'.$rndFilename.'.ico'), true);
					$icon->clear();
				} else {
					...
					foreach ($this->dimensions as $dimension) {
						$tmpFiles[] = Yii::getAlias('@webroot/assets/x/'.$rndFilename.'.'.$dimension.'.png');
						exec('convert -scale '.$dimension.' '.$srcImg.' '.Yii::getAlias('@webroot/assets/x/'.$rndFilename).'.'.$dimension.'.png');
					}
					exec('convert '.implode(' ', $tmpFiles).' '.Yii::getAlias('@webroot/assets/x/'.$rndFilename.'.ico'));
				...
				if ($this->email) {
					Yii::$app->mailer
						...
						->attach(Yii::getAlias('@webroot/assets/x/'.$rndFilename.'.ico'), ['fileName' => 'favicon.ico'])
						->send()
					;
				}
