...
echo Html::beginTag('div', ['class' => 'site-lyrics-albums']);
    ...
    foreach ($data as $album) {
        if ($album->tracks) {
            ...
            echo Html::tag('span', Html::tag('span', $album->year, ['class' => 'badge']) . Html::a($album->name, ['lyrics3tracks', 'artist' => $album->artist->url, 'year' => $album->year, 'album' => $album->url], ['class' => 'ml-1 notranslate']), ['class' => 'h4 text-left float-left mb-0']);
            echo Html::beginTag('div', ['class' => 'float-right']);
            if ($album->buy) {
