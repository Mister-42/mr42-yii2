...
foreach ($data->comments as $comment) {
    ...
    $bar[] = Yii::$app->icon->name('clock')->class('mr-1 text-muted') . Html::tag('time', Yii::$app->formatter->asRelativeTime($comment->created), ['datetime' => date(DATE_W3C, Yii::$app->formatter->asTimestamp($comment->created))]);
    ...
    foreach ($comment->commentReplies as $reply) {
        ...
        $bar[] = Yii::$app->icon->name('clock')->class('mr-1 text-muted') . Html::tag('time', Yii::$app->formatter->asRelativeTime($reply->created), ['datetime' => date(DATE_W3C, Yii::$app->formatter->asTimestamp($reply->created))]);
